...
# ---- Make sure openai and pyttsx3 already imported and openai.api_key set earlier ----

# Simple speak using your engine (replace if you use other speak)
def speak(text):
    try:
        engine.say(text)
        engine.runAndWait()
    except Exception:
        # fallback print so you can see output during debugging
        print("SPEAK:", text)

# Robust ask_gpt that returns the assistant's reply (or None on error)
def ask_gpt(query):
    try:
        # Use the same ChatCompletion approach you already have
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
        ...
        # Print full error for debugging and return None
        print("ask_gpt ERROR:", repr(e))
        return None
...
def open_website(url, name=None):
    ...
    try:
        ...
        speak(f"❌ Error opening {name}. {str(e)}")
...
def set_reminder(time_str, reminder_message):
    ...
    try:
        ...
        speak("❌ Invalid time format. Use HH:MM.")
...
def process_command(query):
    ...
    # ----------- DESKTOP AUTOMATION RECOGNITION ----------- #

    if "mouse click" in query:
        mouse_click()
        return

    if "double click" in query:
        double_click()
        return
